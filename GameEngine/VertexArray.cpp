...
VertexArray::VertexArray(const float* verts, unsigned int vSize, const unsigned int* idxs, unsigned int iSize) : _vSize(vSize), _iSize(iSize)
{
	...
	glBufferData(GL_ARRAY_BUFFER, vSize * 3 * sizeof(float), verts, GL_STATIC_DRAW); // 버퍼 복사
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 3, 0); // 만약 GL_TRUE로 하면 값을 정규화함. int, byte같은 것은 GL_TRUE로 해줘야됨.
}
